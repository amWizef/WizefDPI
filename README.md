...
[![Downloads](https://img.shields.io/github/downloads/amWizef/WizefDPI/total.svg)](https://github.com/amWizef/WizefDPI/releases/)

# SplitWire-Turkey Hakkında Önemli Not (29.07.2025)
>
> [!IMPORTANT]
> Bazı kullanıcılar Discord'a erişmekte veya diğer web sitelerindeki bazı içeriklerin yüklenmemesi hakkında geri bildirimde bulunduğundan farklı metotlarla ancak aynı işlev ile çalışan **[SplitWire-Turkey](https://github.com/cagritaskn/SplitWire-Turkey)** uygulamasını yayınladık. WizefDPI'ı kaldırdıktan sonra ([WizefDPI-Turkey Doğru Şekilde Kaldırma Rehberi](https://github.com/amWizef/WizefDPI/blob/master/REVERT.md)) **[bu aracımızı](https://github.com/cagritaskn/SplitWire-Turkey)** kullanmayı deneyebilirsiniz.
...
- **Asset:** [wizefdpi-0.2.3rc3-turkey.zip](https://github.com/amWizef/WizefDPI/releases/download/release-0.2.3rc3-turkey/wizefdpi-0.2.3rc3-turkey.zip)
- **Full notes:** [CHANGELOG.md](./CHANGELOG.md)

## How to make a new release (and make ZIP download work)

1. Go to the **Actions** tab in this repository.
2. Open workflow: **Release WizefDPI ZIP**.
3. Click **Run workflow** and enter a tag, for example: `release-0.2.3rc4-turkey`.
4. Workflow automatically compiles `wizefdpi.exe`, packages WinDivert dependencies, and creates ZIP:
   - `wizefdpi-<version>.zip` (example: `wizefdpi-0.2.3rc4-turkey.zip`)
5. Workflow then publishes a GitHub Release and uploads that ZIP as a release asset.
...
## WizefDPI — Derin Paket İnceleme (DPI) atlatma aracı (Türkiye versiyonu)

Bu proje tamamen Wizef tarafından yapılmıştır.

Bu uygulama, Türkiye'de bazı internet servis sağlayıcılarının DNS değişikliğine izin vermemesi sebebiyle, bu durumu bertaraf etmek için asıl proje olan [WizefDPI](https://github.com/Wizef/WizefDPI)'ın düzenlenmiş bir versiyonudur.
...
## WizefDPI'ı Kullanmak

WizefDPI'ın Türkiye fork'unu kullanmak için iki yöntem bulunmaktadır.
...
- Batch dosyası ile kullanma: batch dosyası ile kullanmada her defasında elle batch dosyasını başlatarak kullanmanız gerekir (batch penceresi kapatıldığında WizefDPI kullanımına son verilir).

> [!NOTE]
> İndirdiğiniz ZIP dosyasını çıkarttığınız konumdan taşımayın. Kurulacak hizmet .cmd dosyasını çalıştırdığınız dosya yolunu kullanacağından eğer dosyaları taşırsanız hizmet çalışmayacaktır. (Tavsiyem sizi rahatsız etmeyecek bir konuma ZIP dosyasını çıkarmanız ve dosyaları orada saklamanız. Örneğin, ``C:\WizefDPI\``.)

## Hizmet Kurarak Kullanma (Windows başlatılırken otomatik olarak çalıştırılır)

WizefDPI Türkiye versiyonunu hizmet kurarak kullanmak için:

- [wizefdpi-0.2.3rc3-turkey.zip](https://github.com/amWizef/WizefDPI/releases/download/release-0.2.3rc3-turkey/wizefdpi-0.2.3rc3-turkey.zip) dosyasını bilgisayarınıza indirin.
...
> [!NOTE]
> Bu işlem bilgisayarınıza WizefDPI hizmetini kuracaktır. WizefDPI hizmetini bilgisayarınızdan kaldırmak için çıkarttığınız dosyalar içerisindeki ``service_remove.cmd`` dosyasını yönetici olarak çalıştırmanız gerekmektedir.

## Batch Dosyası İle Kullanma (Tek seferlik, pencere kapatıldığında sona erecek şekilde)

WizefDPI Türkiye fork'unu batch dosyasını çalıştırarak kullanmak için **(Bir komut penceresi açılır ve uygulama çalışmaya başlar, bu pencere kapatıldığında çalışmaya son verilir)** :
...
> [!NOTE]
> ``turkey_dnsredir.cmd`` dosyasını yönetici olarak çalıştırdığınızda WizefDPI aktif olacaktır. Ancak bu yöntemle çalıştırıldığında hem bilgisayarınız yeniden başlatıldığında WizefDPI'ı elle açmanız gerekecek, hem de ``turkey_dnsredir.cmd`` ile açılan pencere kapatıldığında WizefDPI deaktive olacaktır.

## WizefDPI'ı Kaldırmak ve DNS Ayarlarını Eski Haline Getirmek

WizefDPI'ı tamamen kapatmak ve silmek, bununla birlikte DNS atamasını kaldırmak için **[bu rehberi](https://github.com/amWizef/WizefDPI/blob/master/REVERT.md)** takip edebilirsiniz.

## Sık Karşılaşılan Sorunlar

- WinDivert dosyaları bulunamadı hatası (Yalancı virüs algılaması):
WinDivert dosyaları bulunamadı hatası alıyorsanız antivirüs programınıza ayıkladığınız klasörü dışlama/istisna olarak ekleyin. Windows Defender kullanıyorsanız [buradaki rehberi](https://support.microsoft.com/tr-tr/windows/windows-g%C3%BCvenli%C4%9Fi-ne-d%C4%B1%C5%9Flama-ekleme-811816c0-4dfd-af4a-47e4-c301afe13b26) (Kaspersky antivirüs programı için [buradaki rehberi](https://support.kaspersky.com/ksos/8.5/tr-TR/227390.htm)) takip ederek "wizefdpi-0.2.3rc3-turkey" klasörünü dışlamalara ekleyebilirsiniz. Ancak dışlamalara eklemek sorununuzu çözmeyebilir. Bu durumda Kaspersky isimli programı sisteminizden tamamen kaldırıp ardından WizefDPI-Turkey'in kurulumunu tekrar gerçekleştirmeniz gerekebilir.

- Hizmetin başlatmaya çalışıldığında "Dosya yolu bulunamadı" hatası:
Bu hata indirdiğiniz .zip klasörünü çıkardığınız konumdan farklı bir konuma taşımanız halinde ya da bazı dosyaları silmeniz halinde ortaya çıkar. Bu durumda [wizefdpi-0.2.3rc3-turkey.zip](https://github.com/amWizef/WizefDPI/releases/download/release-0.2.3rc3-turkey/wizefdpi-0.2.3rc3-turkey.zip) dosyasını tekrar bilgisayarınızda bir konuma çıkararak öncelikle service_remove.cmd dosyasını yönetici olarak çalıştırdıktan sonra seçeceğiniz diğer .cmd dosyasını tekrar çalıştırarak bu sorunu çözebilirsiniz.
...
İnternet bağlantınızı ethernet adaptörü aracılığı ile sağlıyorsanız ve WizefDPI tüm çözümleri denemenize rağmen WizefDPI'ı çalıştaramadıysanız **harmanprecious** isimli kullanıcıya ait çözüm rehberine **[bu adresten](https://www.technopat.net/sosyal/konu/wizefdpi-ethernet-kartiyla-calismiyorsa-ne-yapilmali.3619948/)** adresinden ulaşabilirsiniz.
...
### 3- WizefDPI ile Benzer Programlar
